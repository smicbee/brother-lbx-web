...
import { parseLBX, setObject } from './parser.js';
import { renderToSvg } from './svg.js';
import { pngToQlRasterJob, renderSvgToPng } from './node.js';

const USAGE = 'Usage: lbx-render input.lbx --svg out.svg [--png out.png] [--raster out.bin] [--set name=value]';
...
function parseArgs(argv: string[]) {
  ...
  const input = argv[0];
  const result: { input: string; svg?: string; png?: string; raster?: string; sets: Array<[string, string]> } = { input, sets: [] };
  ...
  if (!result.svg && !result.png && !result.raster) usage();
  return result;
}
...
console.log(JSON.stringify({ input: basename(args.input), objects: document.objects.length, warnings: document.warnings.length, outputs: { svg: args.svg, png: args.png, raster: args.raster } }));
